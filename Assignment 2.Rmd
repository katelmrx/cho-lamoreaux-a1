...
```{r}
# correlation
#  From Kate: Unsure if this number changes each day we run this. Earlier corr in text was -0.1433. I updated it to what I got: -0.1313.
...
-   Which cities (locations) have the highest search frequency for `covid`? (Note that there might be multiple rows for each city if there were hits for keywords in that city. It might be easier to answer this question if we had the search hits info for both search terms in two separate variables. That is, each row would represent a unique city.
    ...
    -   Wapella (100), Oak Lawn (99), Strasberg (90), and Glenview (90) have the highest searches for `covid`.
...
-   Is there a relationship between the search intensities between the two keywords we used?
    ...
    ```{r}
    # correlation
    #  From Kate:This number appears to change each day we run this.
...
```{r}
#Clean NAME in ACS data

#Add location variable to ACS name with city names
```
